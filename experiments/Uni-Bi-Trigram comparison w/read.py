...
# Decoder for slot tagging
decoder = IobSequenceEncoderDecoder()

# Path to the ATIS dataset
DATA_DIR = "../../data/raw_data/ms-cntk-atis"

# Directory for saving the train and test JSON files
...
def load_ds(fname):
    """
    Load dataset from a .pkl file and print dataset statistics.

    Args:
        fname (str): File path to the .pkl dataset.

    Returns:
        tuple: A tuple containing the dataset and its dictionaries.
    """
...
def to_rasa_nlu_format(query, slots, intent, output_file):
    """
    Convert ATIS dataset to Rasa NLU JSON format.

    Args:
        query (list): List of queries (input texts).
        slots (list): List of slot labels.
        intent (list): List of intent labels.
...
# Extract mappings from dictionaries
t2i, s2i, in2i = map(dicts.get, ['token_ids', 'slot_ids', 'intent_ids'])
i2t, i2s, i2in = map(lambda d: {d[k]: k for k in d.keys()}, [t2i, s2i, in2i])

# Convert the training dataset to Rasa format
train_query, train_slots, train_intent = map(train_ds.get, ['query', 'slot_labels', 'intent_labels'])
to_rasa_nlu_format(train_query, train_slots, train_intent, os.path.join(OUTPUT_DIR, 'train.json'))

# Convert the testing dataset to Rasa format
test_query, test_slots, test_intent = map(test_ds.get, ['query', 'slot_labels', 'intent_labels'])
to_rasa_nlu_format(test_query, test_slots, test_intent, os.path.join(OUTPUT_DIR, 'test.json'))

print("Data successfully converted and saved in the train_test_split directory.")
